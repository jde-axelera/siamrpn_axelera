"""
Calibration transform for the siamrpn_r50_l234_dwxcorr_lt template encoder.

Used with axcompile --transform to preprocess calibration images for the
LT template encoder (127×127 input). Matches pysot get_subwindow: BGR float32
CHW [0, 255] — no ImageNet normalisation.

Place this file in the SDK customers/ directory before running axcompile:
    cp lt_tracker/scripts/transform_template.py <SDK_ROOT>/customers/<your_dir>/
"""
import numpy as np
import cv2
import torch
...
def get_preprocess_transform(image) -> torch.Tensor:
    """Preprocess a calibration frame for the LT SiamRPN++ template encoder.

    Accepts a PIL Image (RGB) or an HWC BGR uint8 numpy array.
    Center-crops to square, resizes to 127×127, returns a (3, 127, 127)
...
if 'transform_template' not in _sys.modules:
    _m = _types.ModuleType('transform_template')
    _m.__dict__.update(globals())
    _sys.modules['transform_template'] = _m
