#!/usr/bin/env python3
"""
track_pf_local.py — SiamRPN++ tracker with optional Particle Filter
=====================================================================

Usage:
    conda activate pysot
    python scripts/track_pf_local.py --config configs/track_config.json

Config options (track_config.json):
    use_particle_filter  true  → SiamRPN++ + PF position fusion
                         false → pure SiamRPN++ (tracker output directly)
    max_frames           int   → cap frames; null = full video

Output: 4-panel video
    [Tracked frame + bbox]  [Score map]  [Search crop]  [SAM2 reference]
    Panel 1 also shows particle cloud when use_particle_filter is true.
"""
...
def build_onnx_tracker(c, raw_store, ort, cfg, ModelBuilder, build_tracker):
    cfg.defrost()
...
def main():
    ...
    ort, cfg, ModelBuilder, build_tracker = setup_pysot(c['paths']['pysot_root'])

    raw_store = {}
    tracker, _enc, _zf = build_onnx_tracker(c, raw_store, ort, cfg, ModelBuilder, build_tracker)
    ...
    mode_str = 'SiamRPN++ + PF' if use_pf else 'SiamRPN++ only'
    ...
    # PF state (unused when use_pf=False but kept to avoid branching in viz)
    N          = pf['n_particles']
    ...
    for fi in range(max_frames):
        ...
        # ── Track ─────────────────────────────────────────────────────────────
            ...
            if use_pf:
                ...
                # Estimate from PF; override tracker centre for next crop
                ...
                # Pure SiamRPN++: use tracker output directly
                ...
                if bbox is not None:
                    ...
                    w_out  = float(tracker.size[0])
                    h_out  = float(tracker.size[1])
                ess = float(N)  # not used, set to full for display
        ...
        panel1_label = f'{"PF+" if use_pf else ""}SiamRPN++  score={track_score:.3f}  f={fi}'
        ...
        if fi > 0 and s_x > 0 and use_pf:
            ...
            for pi in np.argsort(weights)[-100:]:
                svx, svy = int(fx_v[pi] * cell + cell / 2), int(fy_v[pi] * cell + cell / 2)
                if 0 <= svx < sq and 0 <= svy < sq:
                    cv2.circle(hmap, (svx, svy), 3, (255, 255, 255), -1)
        psr_col = (0, 230, 100) if psr > tu['min_psr'] else (0, 80, 255)
        p2 = add_label(fit_panel(hmap, TARGET_H, PANEL_W), f'Score map  PSR={psr:.1f}  ESS={ess:.0f}/{N}')

        p3 = add_label(fit_panel(search_bgr, TARGET_H, PANEL_W), f'Search crop  s_x={sx_display:.0f}px')
        ...
        put(info, f"mode={'PF+ONNX' if use_pf else 'ONNX only'}", 480, 38, 0.42, (160,160,160))
        put(info, f"TMPL_PSR={tu['min_psr']}  α={tu['alpha']}", 480,  58, 0.42, (160,160,160))
