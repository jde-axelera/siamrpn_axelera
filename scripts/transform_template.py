"""
Calibration transform for the IR-fine-tuned SiamRPN++ template encoder.

Used with axcompile --transform to preprocess calibration images for the
template encoder (127×127 input). Applies ImageNet mean/std normalisation to
...
def get_preprocess_transform(image) -> torch.Tensor:
    """Preprocess a calibration frame for the SiamRPN++ template encoder.

    Accepts a PIL Image (RGB) or an HWC BGR uint8 numpy array.
    Center-crops to square, resizes to 127×127, normalises with ImageNet
    mean/std, returns a (3, 127, 127) float32 tensor.
    """
